...
### 講師:王大進
---
### 網路診斷與容錯：系統級故障診斷、PMC Model 與條件診斷度
### 一、簡介
...
透過這次網路診斷與容錯的內容，我首先了解到，即使單一處理器具有 99.99% 的可靠度，當系統中的處理器數量增加之後，整體系統的可靠度仍然可能快速下降。從 100、1,000 到 10,000 個處理器的計算結果可以明顯看出，系統規模越大，故障診斷的重要性也越高。

在 System-Level Diagnosis 的部分，我原本認為只要讓節點互相檢查，就可以直接找出故障節點，但是學習到 tester 本身也可能發生故障之後，我發現實際情況比想像中複雜。正常節點進行測試時，測試結果可以被信任，但如果測試節點本身已經故障，那麼產生的結果可能為 0，也可能為 1，因此不能直接相信所有測試結果。

Syndrome 的概念也讓我更容易理解整個故障診斷的過程。單獨看一個測試結果可能無法判斷整個系統的狀態，但如果將多個節點之間的測試結果組合起來，就可以形成一組 Syndrome，再由 Arbiter 進一步分析。不過因為故障節點可能產生任意測試結果，因此同一種故障情況也可能產生多組不同的 Syndrome。

Diagnosable 與 Distinguishable 的部分讓我了解到，系統是否具有診斷能力，並不是只看有沒有取得測試結果，而是要進一步判斷不同的故障集合能不能透過 Syndrome 加以區分。如果兩個不同故障集合的 Syndrome Table 發生重疊，就可能出現相同的測試結果，使系統無法確定真正的故障情況。

t-Diagnosability 則讓我了解到，每一種網路都有自己的診斷能力。當故障節點數量逐漸增加時，故障節點所產生的不可信測試結果也會增加，最後可能使 Arbiter 無法正確判斷。因此，診斷度 `t` 可以用來表示一個網路在最多多少個故障節點的情況下，仍然能夠正確進行故障辨識。

我覺得 Conditional Diagnosability 是這次內容中很有意思的部分。一般 Diagnosability 沒有對故障節點的分布進行限制，但如果加入「一個節點的所有鄰居不能同時全部故障」這項條件，就可以排除許多會產生大量不確定測試結果的情況。如此一來，系統便可以在更多節點故障的情況下維持診斷能力。

以 Hypercube 為例，一般的診斷度為：
...
時，Conditional Diagnosability 可以達到：

$$
4n-7
$$

可以看出加入合理的故障分布條件之後，系統的診斷能力能夠明顯提高。

經過這次內容的整理，我對網路中的故障診斷方式有了更完整的認識，也更加了解 Syndrome、PMC Model、Diagnosability、Distinguishable 與 Conditional Diagnosability 之間的關係。
